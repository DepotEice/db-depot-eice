...
BEGIN
    ...
    SELECT
        ...
        IIF([SenderId] = @UserId, (SELECT CONCAT([FirstName], ' ', [Lastname]) FROM dbo.Users WHERE [Id] = ReceiverId), (SELECT CONCAT([FirstName], ' ', [Lastname]) FROM dbo.Users WHERE [Id] = SenderId)) as 'Username',
        [Content],
        [SentAt]
    FROM dbo.[Messages] m
        INNER JOIN (
